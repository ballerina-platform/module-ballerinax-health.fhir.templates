...
isolated final readonly & r4:FHIRSourceConnectInteraction srcConnectImpl = {
    read: practitionerReadImpl,
    search: practitionerSearchImpl,
    create: practitionerCreateImpl
};
//Default profile is set to International Resource URL.
final readonly & string defaultProfile = "http://hl7.org/fhir/StructureDefinition/Practitioner";

isolated function practitionerSearchImpl(map<r4:RequestSearchParameter[]> params, http:RequestContext ctx) returns r4:BundleEntry[]|r4:FHIRError {
    ...
    lock {
        ...
        PractitionerSourceConnect sourceConnect = profileImpl.get(defaultProfile);
        ...
        r4:Bundle|Practitioner[] practitioners = check sourceConnect.search(params.clone(), fhirContext);
        r4:BundleEntry[] entries = [];

        if practitioners is r4:Bundle {
            entries = practitioners.entry ?: [];
        } else if practitioners is Practitioner[] {
...
isolated function practitionerReadImpl(string id, http:RequestContext ctx) returns r4:FHIRResourceEntity|r4:FHIRError {
    ...
    lock {
        ...
        Practitioner practitioner = check sourceConnect.read(id, fhirContext);
        log:printDebug(string `[ReadImpl] Retrieved resource:  ${practitioner.toJsonString()}`);

        return new (practitioner);
    }
}

isolated function practitionerCreateImpl(r4:FHIRResourceEntity resourceEntity, http:RequestContext ctx) returns string|r4:FHIRError {
    ...
    lock {
        ...
        if resourceRecord is Practitioner {
            ...
            string diagMsg = string `Expected r4:Practitioner FHIR resource model not found. Instead, found a model of type:" ${(typeof resourceRecord).toBalString()}`;
            return r4:createInternalFHIRError("Incoming r4:Practitioner resource model not found", r4:ERROR, r4:PROCESSING_NOT_FOUND, diagnostic = diagMsg);
        }
    }
}

public type PractitionerSourceConnect object {
...
public type ProfileImplementations record {
    map<PractitionerSourceConnect> sourceConnectImplementations;
};
