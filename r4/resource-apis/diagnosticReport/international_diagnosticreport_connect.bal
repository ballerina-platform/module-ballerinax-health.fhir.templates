...
public isolated class InternationalDiagnosticReportSourceConnect {

    *DiagnosticReportSourceConnect;
    isolated function profile() returns r4:uri {
        return "http://hl7.org/fhir/StructureDefinition/DiagnosticReport";
    }
    ...
    isolated function read(string id, r4:FHIRContext ctx) returns DiagnosticReport|r4:FHIRError {
        ...
        if (res is http:ClientError) {
            ...
            if (payload is error) {
                ...
                r4:DiagnosticReport|error fhirResource = payload.cloneWithType(r4:DiagnosticReport);
        ...
        if (res is http:ClientError) {
            ...
            if (payload is error) {
                ...
                if (payload is json[]) {
                    ...
                    DiagnosticReport[] fhirResources = [];
                    foreach var p in payloadArray {
                        r4:DiagnosticReport|error fhirResource = p.cloneWithType(r4:DiagnosticReport);
                        if (fhirResource is error) {
                            r4:FHIRError fhirError = r4:createFHIRError("Did not get a FHIR Resource from source.", r4:CODE_SEVERITY_ERROR, r4:TRANSIENT_EXCEPTION);
                            return fhirError;
                        } else {
